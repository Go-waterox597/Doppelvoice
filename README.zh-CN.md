...
> **你的声音，跨越语言。**
> 端到端实时语音翻译 + 0 样本音色克隆，**支持 9 种语言**（中 / 英 / 日 / 印尼 / 西 / 葡 / 德 / 法 + 中英互译自动模式）。
> 对方在 Zoom / 腾讯会议 / 微信电话 / OBS 等任何接受麦克风的软件里，都能听到**你音色的目标语言**。
>
...
```
你说<源语种>   ─►  Doppelvoice  ─►  对方听到你音色的<目标语种>
 ┌──────────┐     ┌─────────┐     ┌────────────────────────┐
...
源/目标语种从 9 个代码任选（`zh / en / ja / id / es / pt / de / fr`），
或两端都填 `zhen` 走中英互译自动模式。

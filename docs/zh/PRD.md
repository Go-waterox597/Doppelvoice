...
- **v0.2.0**：4 路代码审查全包：9 语言 + 设备去重 + 音色克隆调优 + frozen
  config + 日志脱敏 + 协议安全加固 + 测试 3→46 + PyInstaller Windows 二进制（已完成 2026-04-26）
- **v0.2.1**：元数据 / CI / 文档清理：补 pyproject 缺失依赖、版本号同步、
  GitHub Actions CI、README 双路安装说明（已完成 2026-04-26）
- **v0.2.2**：20 路对抗审查 + 一锅端 bug 修复（CRITICAL: opus 无界 / `_HAS_SOXR`
  缺 import / UTF-8 BOM）+ frozen __post_init__ 校验 + LGPL 合规 + 测试 46→86
  （已完成 2026-04-26）
